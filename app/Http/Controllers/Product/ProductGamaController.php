...
class ProductGamaController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $result = VlProductGama::orWhere('identity','LIKE',$q)
    ...
    public function show(string $id)
    {
        ...
        $result = VlProductGama::all();
        $filename = app(VlProductGama::class)->getTable() . date("_Ymd_His");
    ...
    public function edit(string $id)
    {
        ...
        $row = VlProductGama::whereToken($id)->first();
        abort_if(!$row,403,'Token no valido');
        $log = VlChangeLog::whereTablename(app(VlProductGama::class)->getTable())
    ...
    public function update(Request $request, string $id)
    {
        ...
        $l->tablename   = app(VlProductGama::class)->getTable();
        $l->data_before = $row;
        $l->record_id   = $row->id;
        /* Datos */
        $row->fill($request->all());
    ...
    public function destroy(string $id)
    {
        ...
        if($row && $grant){
            if(
                VlProduct::where('gama_id',$row->gama_id)
                ->get()
                ->count('id')
            ){
                $data['status']     = 102;

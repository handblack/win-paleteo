...
class ProductTenidoController extends Controller
{
    /**
     * Display a listing of the resource.
     */
    private $grantname  = 'pr';
    private $table      = 'tenido';
    public function index(Request $request)
    {
        ...
        $result = VlProductTenido::orWhere('identity','LIKE',$q)
    ...
    public function show(string $id)
    {
        ...
        $result = VlProductTenido::all();
        $filename = app(VlProductTenido::class)->getTable() . date("_Ymd_His");
    ...
    public function edit(string $id)
    {
        ...
        $row = VlProductTenido::whereToken($id)->first();
        abort_if(!$row,403,'Token no valido');
        $log = VlChangeLog::whereTablename(app(VlProductTenido::class)->getTable())
    ...
    public function update(Request $request, string $id)
    {
        ...
        $l->tablename   = app(VlProductTenido::class)->getTable();
        $l->data_before = $row;
        $l->record_id   = $row->id;
        /* Datos */
        $row->fill($request->all());
    ...
    public function destroy(string $id)
    {
        ...
        if($row && $grant){
            if(
                VlProduct::where('tenido_id',$row->tenido_id)
                ->get()
                ->count('tenido_id')
            ){
                $data['status']     = 102;

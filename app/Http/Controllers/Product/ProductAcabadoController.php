...
class ProductAcabadoController extends Controller
{
    /**
     * Display a listing of the resource.
     */
    private $grantname  = 'pr';
    private $table      = 'acabado';
    public function index(Request $request)
    {
        ...
        $result = VlProductAcabado::orWhere('identity','LIKE',$q)
    ...
    public function show(string $id)
    {
        ...
        $result = VlProductAcabado::all();
        $filename = app(VlProductAcabado::class)->getTable() . date("_Ymd_His");
    ...
    public function edit(string $id)
    {
        ...
        $row = VlProductAcabado::whereToken($id)->first();
        abort_if(!$row,403,'Token no valido');
        $log = VlChangeLog::whereTablename(app(VlProductAcabado::class)->getTable())
    ...
    public function update(Request $request, string $id)
    {
        ...
        $l->tablename   = app(VlProductAcabado::class)->getTable();
        $l->data_before = $row;
        $l->record_id   = $row->id;
        /* Datos */
        $row->fill($request->all());
    ...
    public function destroy(string $id)
    {
        ...
        if($row && $grant){
            if(
                VlProduct::where('acabado_id',$row->acabado_id)
                ->get()
                ->count('acabado_id')
            ){
                $data['status']     = 102;

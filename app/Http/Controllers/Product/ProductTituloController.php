...
class ProductTituloController extends Controller
{
    /**
     * Display a listing of the resource.
     */
    private $grantname  = 'pr';
    private $table      = 'titulo';
    public function index(Request $request)
    {
        ...
        $result = VlProductTitulo::orWhere('identity','LIKE',$q)
    ...
    public function show(string $id)
    {
        ...
        $result = VlProductTitulo::all();
        $filename = app(VlProductTitulo::class)->getTable() . date("_Ymd_His");
    ...
    public function edit(string $id)
    {
        ...
        $row = VlProductTitulo::whereToken($id)->first();
        abort_if(!$row,403,'Token no valido');
        $log = VlChangeLog::whereTablename(app(VlProductTitulo::class)->getTable())
    ...
    public function update(Request $request, string $id)
    {
        ...
        $l->tablename   = app(VlProductTitulo::class)->getTable();
        $l->data_before = $row;
        $l->record_id   = $row->id;
        /* Datos */
        $row->fill($request->all());
    ...
    public function destroy(string $id)
    {
        ...
        if($row && $grant){
            if(
                VlProduct::where('titulo_id',$row->titulo_id)
                ->get()
                ->count('family_id')
            ){
                $data['status']     = 102;

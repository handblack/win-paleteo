...
class ProductTejidoController extends Controller
{
    /**
     * Display a listing of the resource.
     */
    private $grantname  = 'pr';
    private $table      = 'tejido';
    public function index(Request $request)
    {
        ...
        $result = VlProductTejido::orWhere('identity','LIKE',$q)
    ...
    public function show(string $id)
    {
        ...
        $result = VlProductTejido::all();
        $filename = app(VlProductTejido::class)->getTable() . date("_Ymd_His");
    ...
    public function edit(string $id)
    {
        ...
        $row = VlProductTejido::whereToken($id)->first();
        abort_if(!$row,403,'Token no valido');
        $log = VlChangeLog::whereTablename(app(VlProductFamilia::class)->getTable())
    ...
    public function update(Request $request, string $id)
    {
        ...
        $l->tablename   = app(VlProductTejido::class)->getTable();
        $l->data_before = $row;
        $l->record_id   = $row->id;
        /* Datos */
        $row->fill($request->all());

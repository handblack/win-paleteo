...
class ProductPresentacionController extends Controller
{
    /**
    ...
    public function index(Request $request)
    {
        ...
        $result = VlProductPresentacion::orWhere('identity','LIKE',$q)
    ...
    public function edit(string $id)
    {
        ...
        $row = VlProductPresentacion::whereToken($id)->first();
        abort_if(!$row,403,'Token no valido');
        $log = VlChangeLog::whereTablename(app(VlProductPresentacion::class)->getTable())
    ...
    public function update(Request $request, string $id)
    {
        ...
        $l->tablename   = app(VlProductPresentacion::class)->getTable();
        $l->data_before = $row;
        $l->record_id   = $row->id;
        /* Datos */
        $row->fill($request->all());
    ...
    public function destroy(string $id)
    {
        ...
        if($row && $grant){
            if(
                VlProduct::where('presentacion_id',$row->presentacion_id)
                ->get()
                ->count('presentacion_id')
            ){
                $data['status']     = 102;

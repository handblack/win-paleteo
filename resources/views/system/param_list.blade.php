...
@section('breadcrumb')
    <div class="content-header pb-0">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0"><i class="fas fa-cubes fa-fw"></i> Parametros</h1>
                </div>
                <div class="col-sm-6">
                ...
                <tbody>
                    @forelse ($result as $item)
                        <tr id="tr-{{ $item->id }}">
                            ...
                            <td class="{{ $item->isactive == 'N' ? 'tachado' : '' }}">{{ $item->value }}</td>
                            <td class="{{ $item->isactive == 'N' ? 'tachado' : '' }}">{{ $item->id2 }}</td>
							<td class="text-right text-nowrap">
								<a href="{{ route('parameter.edit',$item->token) }}">
									<i class="far fa-edit"></i>
								</a> |
								<a href="#" class="delete-record" data-id="{{ $item->id }}" data-url="{{ route('parameter.destroy',$item->token) }}">

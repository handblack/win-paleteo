...
@section('breadcrumb')
    <div class="content-header pb-0">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0"><i class="fas fa-cubes fa-fw"></i> Secuenciador</h1>
                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item">Sistema</li>
                        <li class="breadcrumb-item active">Secuenciador</li>
                    </ol>
                </div>
            </div>
        </div>
                        ...
                        <div class="btn-toolbar" role="toolbar">
                            <div class="input-group">
                                ...
                                <div class="input-group-append">
                                    ...
                                    <a href="{{ route('familia.create') }}" class="btn btn-success">
        ...
        <div class="card-body table-responsive p-0">
            <table class="table table-hover table-sm">
                <thead>
					...
					<th width="130" class=" text-right">ULTIMO</th>
                ...
                <tbody>
                    @forelse ($result as $item)
                        <tr id="tr-{{ $item->id }}">
                            <td class="{{ $item->isactive == 'N' ? 'tachado' : '' }}">{{ $item->doctype->doctypecode }}-{{ $item->serial }}</td>
                            <td class="{{ $item->isactive == 'N' ? 'tachado' : '' }} text-right">{{ $item->lastnumber }}</td>
                            <td class="{{ $item->isactive == 'N' ? 'tachado' : '' }}">{{ $item->doctype->doctypename }}</td>
